...
/* The in-dashboard job detail + apply page — same content as app/jobs/[id],
   but kept inside the dashboard shell so a recruiter who got here from the
   sidebar's "Browse jobs" never leaves the dashboard chrome. */
export default function DashboardJobDetailPage() {
  ...
  return (
    <div>
      <Link href="/dashboard/jobs" className="text-sm font-semibold text-muted hover:text-ink">
        ← All jobs
      </Link>

      {state === "loading" ? (
        <div className="mt-6 grid h-64 place-items-center rounded-2xl border border-line bg-white">
          <Loader />
        </div>
      ) : state === "missing" || !job ? (
