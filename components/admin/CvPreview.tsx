 ...
 *
 * Two formats, two mechanisms, because browsers can only render one of them:
 *
 ...
 *
 * The conversion is deliberately client-side. The obvious shortcut — pointing
 * a third-party viewer (Google/Office) at the file URL — would mean uploading
 * a candidate's CV to someone else's servers. These are signed, expiring links
 * to personal data; they don't get handed to another company.
 *
 * .doc (the pre-2007 binary format) has no such converter. It downloads. */
...
export function canPreview(contentType: string): boolean {
  return contentType === PDF || contentType === DOCX;
}
...
export function CvPreview({
  ...
  return (
    <Unavailable
      url={url}
      reason="Word 97–2003 (.doc) files can't be previewed in a browser."
    />
  );
}
